...
for train_index, val_index in tqdm(cv.split(X_train, y_train), total=cv.get_n_splits(), desc="Cross-validation"):
    ...
    val_predictions = nb_model.predict(X_cv_val)
...
with open("results/NaiveBayesResults.txt", "w") as f:
...
with open("results/NaiveBayesResults.txt", "a") as f:
...
# np.savetxt("results/nb_test_predictions_with_embeddings.csv", test_results, delimiter=",", fmt="%.6f")
# print("Predictions and embeddings for the test set have been saved to 'results/nb_test_predictions_with_embeddings.csv'.")
